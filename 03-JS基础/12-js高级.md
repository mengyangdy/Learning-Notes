...
var object = {　　　　
  name: "My Object",
  getNameFunc: function () {
      //在这里的this还是值得object这个对象 把这个对象的this 赋值给that
    var that = this;
    return function () {
        //返回的就是object中的name
      return that.name;
...
function fn1 () {
  ...
  fn2()
  console.log('fn1')// 8. fn1
}

function fn2 () {
  console.log(222) // 2. 222
  fn3()
  console.log('fn2')// 7. fn2
}

function fn3 () {
  console.log(333)//3. 333
  fn4()
  console.log('fn3')// 6. fn3
}

function fn4 () {
  console.log(444) // 4.444
  console.log('fn4')//5. fn4
}
...
```javascript
//^表示以什么开头$表示一什么结尾 \d表示匹配数字{11}出现11次
^\d{11}$
```
...
```javascript
// 匹配字母数字下划线出现一次到多次 @ 匹配字母数字下划线出现一次到多次 \.转译点  匹配字母数字下划线出现一次到多次
^\w+@\w+\.\w+$
```
